...
function DatasetEditModalController($q, config, ParticipantService, DatasetMetadataService, $modalInstance, participantId, entry, datasetName, onSave) {
  ...
  self.entry = entry;
  self.onSave = onSave;
  self.isAutoGeneratedId = isAutoGeneratedId;
  ...
  function activate(){
    ...
    if(self.isNewSubject && isAutoGeneratedId()) {
  ...
  function isAutoGeneratedId() {
    return config.subjectIdPrefix !== undefined;
  }

  function isDisabled(column) {
    // disable primary ID.
    if(column.dataIndex === config.subjectNoun){
      // disable if it's an existing record, or if it's been auto-generated
      return isAutoGeneratedId() || !self.isNewSubject;
    }
    return false;
  }
